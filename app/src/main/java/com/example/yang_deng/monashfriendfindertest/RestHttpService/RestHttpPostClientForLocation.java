...
public class RestHttpPostClientForLocation {
    private static final String BASE_URI = "http://10.0.0.24:8080/TESTFOR5046ASS/webresources";

    public static void createStudentLocation(StudentLocation studentlocation){ //initialise
        URL url = null;
        HttpURLConnection conn = null;
        final String methodPath="/test.studentlocation/";
        try {
            /*Gson gson =new Gson();*/
            Gson gson = new GsonBuilder().setDateFormat("yyyy-MM-dd'T'HH:mm:ss").create();
            String stringStudentProfileJson=gson.toJson(studentlocation);
